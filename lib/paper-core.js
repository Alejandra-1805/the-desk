...
export async function paperScan(){
  ...
  for(const agent of AGENTS){
    ...
    event.reference_price_usd=chosen?.priceUsd||null;
    const {data:savedEvent,error}=await supabase.from("agent_events").insert(event).select("id").single();
    if(error) throw error;

    if(d.decision==="BUY" && chosen){
      const {data:existing,error:existingError}=await supabase
        .from("paper_positions")
        .select("id")
        .eq("agent_id",agent.id)
        .eq("token_mint",chosen.mint)
        .eq("status","OPEN")
        .maybeSingle();
      ...
      if(!existing){
        const {error:positionError}=await supabase.from("paper_positions").insert({
          agent_id:agent.id,
          decision_event_id:savedEvent.id,
          token_mint:chosen.mint,
          token_symbol:chosen.symbol,
          entry_price_usd:chosen.priceUsd,
          current_price_usd:chosen.priceUsd,
    ...
    await supabase.from("agents").update({status:d.decision==="BUY"?"PAPER POSITION":"SCANNING",updated_at:new Date().toISOString()}).eq("id",agent.id);
