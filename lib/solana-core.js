...
export async function executeBuy({agentId,mint,amountSol}){
  ...
  const defaultAllowlist=process.env.AUTO_EXECUTION_ENABLED==="true"?"bull,degen,quant,bear":"bull";
  const liveAllowlist=(process.env.LIVE_AGENT_ALLOWLIST||defaultAllowlist).split(",").map(x=>x.trim().toLowerCase()).filter(Boolean);
  if(!liveAllowlist.includes(agentId)) throw new Error("Agent is not enabled for the first live test");
...
export async function executeSell({agentId,mint}){
  ...
  const defaultAllowlist=process.env.AUTO_EXECUTION_ENABLED==="true"?"bull,degen,quant,bear":"bull";
  const liveAllowlist=(process.env.LIVE_AGENT_ALLOWLIST||defaultAllowlist).split(",").map(x=>x.trim().toLowerCase()).filter(Boolean);
  if(!liveAllowlist.includes(agentId)) throw new Error("Agent is not enabled for live execution");
