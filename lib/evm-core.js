...
export function signerSecretForAgent(agentId){
  const id=String(agentId||"").toUpperCase();
  const candidates=[
    "AGENT_"+id+"_EVM_SECRET_KEY",
    "AGENT_"+id+"_PRIVATE_KEY",
    "AGENT_"+id+"_SECRET_KEY",
    id+"_PRIVATE_KEY"
  ];
...
export function verifyAgentSigner(agentId){
  ...
  if(!found.value) return {configured:false,address:null,matches:false,source:null,error:"secret_not_found"};
  try{
    const normalized=found.value.startsWith("0x")?found.value:"0x"+found.value;
    const wallet=new Wallet(normalized);
    const matches=!configured || wallet.address.toLowerCase()===configured.toLowerCase();
    return {configured:true,address:wallet.address,matches,source:found.source,error:matches?null:"address_mismatch"};
  }catch{
    return {configured:false,address:null,matches:false,source:found.source,error:"invalid_private_key_format"};
  }
}
